...
describe('CodexCliSessionManager', () => {
  ...
  it('TTL 초과 시 null 반환 + 자동 삭제', () => {
    manager.set('client-1', 'thread-abc', 'gpt-5.5');
    vi.advanceTimersByTime(1500); // 1.5s, exceeds TTL
  ...
  it('모델이 다르면 세션 무효화 + null 반환', () => {
    ...
    // Subsequent lookup with original model also fails (invalidated)
    expect(manager.get('client-1', 'gpt-5.5')).toBeNull();
  });
  ...
  it('get 시 lastUsedAt 갱신 → TTL 연장', () => {
    ...
    expect(manager.get('client-1', 'gpt-5.5')).not.toBeNull(); // Refreshes lastUsedAt
    vi.advanceTimersByTime(700); // Cumulative 1.4s, but refreshed by previous get
